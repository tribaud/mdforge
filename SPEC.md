...
- [x] **Barre d'outils de sélection** (gras/italique/barré/code + P/H1-H4/quote/liste, toggles, raccourcis en tooltip)
...
- [x] **Résultat de recherche d'atelier → la note s'ouvre sur l'occurrence** : le terme est déduit de ce que les occurrences ont en commun (VS Code ne transmet ni la position ni la requête à un éditeur personnalisé), toutes les occurrences sont surlignées, `F3` / `Maj+F3` les parcourent — y compris quand le webview n'a pas le clavier, les raccourcis étant aussi contribués côté VS Code
